...
def generate_config_file(gcn_file_path, parameter, config_file, train_subg, dev_subg, test_subg):
    ...
    config_dic['data_folder'] =  gcn_file_path + '/'
    config_dic['model_folder'] =  gcn_file_path + '/model/'
    os.makedirs(gcn_file_path + '/model/', exist_ok=True)
    ...
    config_dic['save_model_file'] =  'best_model_entqtkg_' + parameter
    config_dic['pred_file'] =  'pred_entqtkg_'  + parameter
    config_dic['load_model_file'] =  'best_model_entqtkg_' + parameter
...
if __name__ == "__main__":
    ...
    config_path = gcn_file_path + '/config'
    result_path = gcn_file_path + '/result'
    ...
    generate_config_file(gcn_file_path, parameter, config_file, train_subgraph, dev_subgraph, test_subgraph)
    ...
    dev_re = open(dev_re_fp, 'w', encoding='utf-8')
    compare_pr(pred_kb_file, eps_kb, dev_re)
    dev_re.close()
    # result on test set
    test_acc = test(GCN_CFG)
    test_re = open(test_re_fp, 'w', encoding='utf-8')
    pred_kb_file = GCN_CFG['model_folder'] + GCN_CFG['pred_file']
    compare_pr(pred_kb_file, eps_kb, open(test_re_fp, 'w', encoding='utf-8'))
    test_re.close()
